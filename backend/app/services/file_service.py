...
class FileService:
    ...
    def __init__(
        ...
        file_crud: FileCrud,
        user_crud: UserCrud,
        file_archive_path: str = Provide["config.file_archive.path"],
    ) -> None:
    ...
    @staticmethod
    async def save_file(save_path: str, file: IO) -> None:
        async with aiofiles.open(save_path, "wb") as saved_file:
            await saved_file.write(file.read())

    @Transactional(propagation=Propagation.REQUIRED)
    async def create_file(self, file: UploadFile, user_id: str) -> FileDTO:
        # TODO : user_id는 임시로 받는 것으로, 추후 session에서 끌고오도록 변경 필요

        if (
            await self.user_crud.get_user(user_id) is None
        ):  # [TODO] Session에서 가져올 user_id가 실존하는지를 검증하는 것이 의미가 있는지 모르겠어서 논의 필요
